...
void cdclog_show(void)
{
	...
	for(i=0; i<logp; i++){
		...
		int t = (d>>12)&0x0f;
		if(t<=3){
			printk("log%d:  rp=%3d  wp=%3d\n", t, d&0x0fff, d>>16);
			if(t==3){
				printk("\n");
			}
		}else{
			printk("log%d:  hp=%3d  sz=%3d\n", t, d&0x0fff, d>>16);
		}
	}
	logp = 0;
}
...
static void cdc_send_start(CDC_UART *cu)
{
	...
	if(cu->ctx_rp == cu->ctx_wp){
		...
		usbd_ep_start_write(cu->in_ep.ep_addr, cu->usb_txbuf+cu->ctx_rp, length);
		cu->ctx_en = 1;
		int nextrp = cu->ctx_rp + length;
		if(nextrp&3){
			// 数据将全部传输完。对齐写指针。
			nextrp = (nextrp+3)&~3;
			cu->ctx_wp = nextrp;
		}
		cu->ctx_nextrp = nextrp;
	}else{
		// 数据在txbuf两端. [........w         r........]
		...
		if(length>USB_PACKET_SIZE){
			length = USB_PACKET_SIZE;
			cu->ctx_nextrp = cu->ctx_rp + USB_PACKET_SIZE;
		}
		usbd_ep_start_write(cu->in_ep.ep_addr, cu->usb_txbuf+cu->ctx_rp, length);
		cu->ctx_en = 1;
	}
...
static void cdc_txbuf_put(CDC_UART *cu, uint8_t *data, int length)
{
	...
	if(cu->ctx_wp < cu->ctx_rp){
		...
		if(length>0){
			if(length >= cu->ctx_rp)
				length = cu->ctx_rp-1;
			memcpy(cu->usb_txbuf, data, length);
			cu->ctx_wp = length;
		}
	}
...
void usbd_cdc_acm_set_line_coding(uint8_t intf, struct cdc_line_coding *line_coding)
{
	...
	memcpy(&cu->linecoding, line_coding, sizeof(struct cdc_line_coding));
